...
const AgentQuoteCreate      = lazy(() => import('./pages/agent/AgentQuoteCreate'));
const AdminAgentAssignments = lazy(() => import('./pages/admin/AdminAgentAssignments'));
const AdminAgentTargets     = lazy(() => import('./pages/admin/AdminAgentTargets'));
...
function RedirectAuthenticated() {
  ...
  if (profile.is_agent) return <Navigate to="/agent" replace />;
...
function RequireAgent({ children }: { children: React.ReactNode }) {
  ...
  if (loading) return <LoadingScreen />;
  if (!user) return <Navigate to="/auth" replace />;
  if (!profile?.is_agent) return <Navigate to="/" replace />;
  return <>{children}</>;
}
...
function AppRoutes() {
  return (
    <Routes>
      ...
      {/* Admin — Affectation des comptes aux agents commerciaux */}
      <Route
        path="/admin/agents"
        element={
          <RequireAdmin>
            <AdminLayout breadcrumbs={[{ text: 'Admin', href: '/admin' }, { text: 'Agents commerciaux', href: '/admin/agents' }]}>
      ...
      {/* Admin — Objectifs des agents commerciaux */}
      <Route
        path="/admin/agents/targets"
        element={
          <RequireAdmin>
            <AdminLayout breadcrumbs={[{ text: 'Admin', href: '/admin' }, { text: 'Agents commerciaux', href: '/admin/agents' }, { text: 'Objectifs', href: '/admin/agents/targets' }]}>
              <AdminAgentTargets />
            </AdminLayout>
          </RequireAdmin>
        }
